 ...
 */
 define(function() {
TimeSpan = function (time, compareFunc) {
    this.days = 0;
    this.hours = 0;
    this.minutes = 0;
    this.seconds = 0;
    ...
    var orient = (time < 0) ? -1 : +1;
    this.milliseconds = Math.abs(time);

    compareFunc = compareFunc || Math.floor;

    this.days = compareFunc(this.milliseconds / (24 * 60 * 60 * 1000)) * orient;
    this.milliseconds = this.milliseconds % (24 * 60 * 60 * 1000);

    this.hours = compareFunc(this.milliseconds / (60 * 60 * 1000)) * orient;
    this.milliseconds = this.milliseconds % (60 * 60 * 1000);

    this.minutes = compareFunc(this.milliseconds / (60 * 1000)) * orient;
    this.milliseconds = this.milliseconds % (60 * 1000);

    this.seconds = compareFunc(this.milliseconds / 1000) * orient;
 ...
 */
var TimePeriod = function (from, to, compareFunc) {
    this.years = 0;
    this.months = 0;
    this.days = 0;
    this.hours = 0;
    ...
    // startDate and endDate as arguments
        ...
        if (diff !== 0) {
            var ts = new TimeSpan(diff, compareFunc);
        ...
        if (this.months >= 1 && this.months < 11 && this.days > 14) {
            this.months += 1;
        }

        if (this.years >= 1 && (this.months > 5)) {
            this.years += 1;
        }
	...
	return {TimePeriod:TimePeriod,TimeSpan:TimeSpan};
});
